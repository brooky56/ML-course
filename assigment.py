...
def load_file(filepath):
    ...
    """
    data_set_file = open(filepath)
    # TODO: read the file from the filepath, keep in mind the file doesn't have a header and it is
    dataframe = pd.read_csv(data_set_file, header=None, delim_whitespace=True, skipinitialspace=True)
...
def define_models(models=dict(), params=dict()):
    ...
    # TODO: create Log Reg model with parameter solver: lbfgs, multi_class auto, max_iter:500
    models['log_reg'] = LogisticRegression(solver='lbfgs', multi_class='auto', max_iter=500)

    # TODO: create a dict of params with multi_class with two options ovr, multinomial
    params['log_reg'] = {'multi_class': ('ovr', 'multinomial')}

    # TODO: create SVC model with parameter gamma:scale
    models['svm'] = SVC(gamma='scale')

    # TODO: create a dict of params with kernel with two options linear, poly
    params["svm"] = {'kernel': ('linear', 'poly')}
...
def evaluate_model(trainX, trainy, testX, testy, model, class_names, title):
    ...
    temp = model.fit(trainX, trainy)

    # make predictions
    # TODO: make predictions for the test data
    yhat = temp.predict(testX)

    # evaluate predictions
    # TODO: evaluate predictions using accuracy on test data
...
def evaluate_models(trainX, trainy, testX, testy, models, class_names):
    ...
    for name, model in models.items():
        ...
        print("Time spent: {0}".format(finish_time))
    return results
...
def plot_confusion_matrix(y_true, y_pred, classes,
    ...
    # We can use this to get precision recall and other useful metrics
    ...
    fig.tight_layout()
    # You can uncomment to see the plots of confusion matrix
    # plt.show()
    return ax
